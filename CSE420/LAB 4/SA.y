...
statement : var_declaration
		...
		| FOR LPAREN expression_statement expression_statement expression RPAREN statement
		{
			...
			ForNode* forNode = new ForNode(
				// The first two nodes are expression_statement nodes
				(ExprStmtNode*)$3->get_ast_node(), // Changed
				(ExprStmtNode*)$4->get_ast_node(), // Changed
...
int main(int argc, char *argv[])
{
	...
	string errorfile_name = "error.txt";
	string codefile_name = "code.txt";

	outlog.open(logfile_name, ios::trunc);
	outerror.open(errorfile_name, ios::trunc);
	outcode.open(codefile_name, ios::trunc);
	...
	cout << endl << "==== Pass 1: Parsing input and building AST ====" << endl;
	outlog << "==== Pass 1: Parsing input and building AST ====" << endl << endl;
	...
	if (errors == 0 && ast_root) {
		cout << "==== Pass 2: Generating Three-Address Code from AST ====" << endl << endl;
		...
		cout << "Three-Address Code Generation Complete. Output written to " << codefile_name << endl;
	} else {
		cout << "Three-Address Code generation skipped due to errors\n" << endl;

...
public:
    ...
    void enter_scope(ofstream &outlog)
    {
        ...
        outlog << "New ScopeTable with ID " << curr_scope->getID() << " created" << endl
               << endl;
    ...
    {
        outlog << "Scopetable with ID " << curr_scope->getID() << " removed" << endl
               << endl;
    ...
    {
        if (curr_scope->Insert_in_scope(name, type))
            return true;
        else
            return false;
    }
    ...
    {
        if (curr_scope->Delete_from_scope(name))
            return true;
        else
            return false;
    }

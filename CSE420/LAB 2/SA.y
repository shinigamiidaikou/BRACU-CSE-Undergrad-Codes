...
func_definition : type_specifier ID LPAREN parameter_list RPAREN dummy11 compound_statement
		{
			...
			// Done in dummy11 production rule
		}
		| type_specifier ID LPAREN RPAREN dummy12 compound_statement
		{
			...
			// Done in dummy12 production rule
		}
		;

dummy11 :
		{
			// outlog<<"At line no: "<<lines<<" dummy11 :  "<<endl<<endl;
			// outlog<<"$0->get_name(): "<<$0->get_name()<<endl<<endl;
			// outlog<<"$-1->get_name(): "<<$-1->get_name()<<endl<<endl;
			// outlog<<"$-2->get_name(): "<<$-2->get_name()<<endl<<endl;
			// outlog<<"$-3->get_name(): "<<$-3->get_name()<<endl<<endl;
			// outlog<<"$-4->get_name(): "<<$-4->get_name()<<endl<<endl;
			symbol_info* func_symbol = new symbol_info($-3->get_name(), $-3->get_type());
			func_symbol->set_as_function($-4->get_name(), func_params);
...
dummy12 :
		{
			// outlog<<"At line no: "<<lines<<" dummy12 :  "<<endl<<endl;
			symbol_info* func_symbol = new symbol_info($-2->get_name(), $-2->get_type());
			func_symbol->set_as_function($-3->get_name(), func_params);
			sym_table->insert(func_symbol);
		}
		;
...
parameter_list : parameter_list COMMA type_specifier ID
		{
			...
			func_params.push_back({$3->get_name(), $4->get_name()});
		}
		| parameter_list COMMA type_specifier
		{
            ...
            // store the necessary information about the function parameters
            // They will be needed when you want to enter the function into the symbol table
			func_params.push_back({$3->get_name(), ""});
		}
 		| type_specifier ID
 		{
            ...
            // store the necessary information about the function parameters
            // They will be needed when you want to enter the function into the symbol table
			func_params.push_back({$1->get_name(), $2->get_name()});
		}
		| type_specifier
		{
            ...
            // store the necessary information about the function parameters
            // They will be needed when you want to enter the function into the symbol table
			func_params.push_back({$1->get_name(), ""});
		}
 		;

compound_statement : LCURL dummy2 statements RCURL
		{
...
declaration_list : declaration_list COMMA ID
		...
		symbol_info* var_symbol = new symbol_info($3->get_name(), "ID");
		var_symbol->set_data_type(var_type);
		var_list.push_back(var_symbol);
		...
		symbol_info* var_symbol = new symbol_info($3->get_name(), "ID");
		var_symbol->set_data_type(var_type);
		var_symbol->set_as_array(stoi($5->get_name()));
		var_list.push_back(var_symbol);
		...
		symbol_info* var_symbol = new symbol_info($1->get_name(), "ID");
		var_symbol->set_data_type(var_type);
		var_list.push_back(var_symbol);
		...
		symbol_info* var_symbol = new symbol_info($1->get_name(), "ID");
		var_symbol->set_data_type(var_type);
		var_symbol->set_as_array(stoi($3->get_name()));
		var_list.push_back(var_symbol);
		}
		;
